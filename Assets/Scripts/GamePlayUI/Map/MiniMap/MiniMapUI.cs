...
public class MiniMapUI : MonoBehaviour
{
    ...
    private void ApplyCell(MiniMapCellUI cellUI, SectorMapCellSnapshot snapshot)
    {
        ...
        {
            cellUI.SetStartSector(GetStartSectorColor(), GetCellAlpha(snapshot));
            return;
        }

        if (snapshot.isLocked || !snapshot.isRevealed)
        {
        ...
        bool showRatios = snapshot.isRevealed && !HasNamedOrBoss(snapshot);
    ...
    private Color GetStartSectorColor()
    {
        return _settings != null ? _settings.StartSectorColor : _neutralColor;
    }
