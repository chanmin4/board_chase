...
public class VSplatterAimAction : MonoBehaviour
{
    ...
    private void OnEnable()
    {
        ...
        if (_inputReader != null)
            _inputReader.SpecialShootEvent += OnMiddleClickRequested;
    ...
    {
        float fallback = kind == FireKind.Paint
        ...
        if (!_bulletLoadout.TryGetActiveBullet(ToAmmoType(kind), out BulletSO bullet))
            return Mathf.Max(0.01f, fallback);
