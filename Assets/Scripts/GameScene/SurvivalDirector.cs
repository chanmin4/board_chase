...
/// <summary>
/// SurvivalDirector (Zones Disabled / API kept for compatibility)
/// - Zone 스폰/상호작용/TTL 로직은 임시 제거(=FPS 전환 작업용)
/// - 오염(Contamination) 그리드/유틸만 유지
/// - 다른 스크립트들이 컴파일되도록 Zone 관련 이벤트/튜닝 필드/스텁 메서드는 "선언만" 유지
/// </summary>
...
/*
    // Zone 관련 이벤트(컴파일 호환용으로 선언만 유지, zonesEnabled=false면 Invoke 안 함)
    public event Action<int> OnZonesResetSeq;
    public event Action<int> OnZoneExpired;
    public event Action OnZonesReset;
    public event Action<int, float> OnZoneProgress;
    public event Action<int, Vector3, float> OnZoneContaminatedCircle;
    public event Action<int> OnZoneConsumed;
    public event Action<int, int, int, bool> OnZoneHit;
    public event Action<int, float, float> OnZoneBonusSectorChanged;
    public event Action ZoneNormalHit_SFX;
    public event Action ZoneCritHit_SFX;
*/
