...
public class AttackBullet : MonoBehaviour
{
    ...
    private int _penetrationClass;
    private float _vaccineMarkAmountOnHit;
    private float _virusMarkAmountOnHit;
    private float _infectionDamageOnHit;
    private float _armorHealthDurabilityLossMultiplier = 1f;
    private float _armorInfectionDurabilityLossMultiplier = 0.5f;
        ...
        ShootHitConfirmedEventChannelSO shootHitConfirmedEvent)
    {
        ...
        _penetrationClass = Mathf.Max(0, penetrationClass);
        _vaccineMarkAmountOnHit = Mathf.Max(0f, vaccineMarkAmountOnHit);
        _virusMarkAmountOnHit = Mathf.Max(0f, virusMarkAmountOnHit);
        _infectionDamageOnHit = Mathf.Max(0f, infectionDamageOnHit);
        _armorHealthDurabilityLossMultiplier = Mathf.Max(0f, armorHealthDurabilityLossMultiplier);
        _armorInfectionDurabilityLossMultiplier = Mathf.Max(0f, armorInfectionDurabilityLossMultiplier);
    ...
    private void TryApplyDamage(RaycastHit hit)
    {
        ...
        {
            Debug.Log(
                $"[AttackBullet] target={damageable.name}, damage={_damage}, infection={_infectionDamageOnHit}",
                damageable);
        }
        ...
        if (_damage > 0f)
        {
            damageable.ReceiveAnAttack(
                _damage,
                _source,
                _penetrationClass,
                _armorHealthDurabilityLossMultiplier);
            appliedAny = true;
        }

        if (TryApplyPaintMark(damageable))
            appliedAny = true;

        if (TryApplyInfectionDamage(damageable))
            appliedAny = true;
    ...
    private bool TryApplyPaintMark(Damageable damageable)
    {
        if (damageable == null ||
        ...
        float markMultiplier =
            damageable.ResolvePaintMarkDamageMultiplierForPenetration(_penetrationClass);

        if (_vaccineMarkAmountOnHit > 0f)
            mark.AddMark(PaintMarkFaction.Vaccine, _vaccineMarkAmountOnHit * markMultiplier);

        if (_virusMarkAmountOnHit > 0f)
            mark.AddMark(PaintMarkFaction.Virus, _virusMarkAmountOnHit * markMultiplier);

        return true;
    }

    private bool TryApplyInfectionDamage(Damageable damageable)
    {
        if (damageable == null || _infectionDamageOnHit <= 0f)
            return false;

        float infectionAmount =
            _infectionDamageOnHit *
            damageable.ResolveInfectionDamageMultiplierForPenetration(_penetrationClass);

        if (infectionAmount <= 0f)
            return false;

        damageable.ApplyArmorInfectionDurabilityDamage(
            _infectionDamageOnHit,
            infectionAmount,
            _armorInfectionDurabilityLossMultiplier);

        PlayerShooterInfection playerInfection =
            damageable.GetComponent<PlayerShooterInfection>() ??
            damageable.GetComponentInParent<PlayerShooterInfection>() ??
            damageable.GetComponentInChildren<PlayerShooterInfection>(true);

        if (playerInfection != null)
        {
            playerInfection.AddInfection(infectionAmount);
            return true;
        }

        EnemyShooterInfection enemyInfection =
            damageable.GetComponent<EnemyShooterInfection>() ??
            damageable.GetComponentInParent<EnemyShooterInfection>() ??
            damageable.GetComponentInChildren<EnemyShooterInfection>(true);

        if (enemyInfection == null)
            return false;

        enemyInfection.AddInfection(infectionAmount);
        return true;
    }
